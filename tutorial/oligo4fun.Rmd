...
In the following example, we use the _acsA_ (acetyl-coenzyme A synthetase) gene data from _Rhodopirellula_ species used in Zure et al. 2016, which is loaded together with the library and used to perform a Minimum Entropy Decomposition ([MED](http://merenlab.org/projects/med/)) analysis, the unsupervised flavour of [oligotyping](http://merenlab.org/projects/oligotyping/).

First we load the package (including example data):
```{r}
library(oligo4fun)
```

Let's have a look at the loaded data, first at the entropy file:
```{r}
entropy
```
...
One can read any fasta file using "read.dna" function from [ape](https://cran.r-project.org/web/packages/ape/index.html) package:
```{r, eval=FALSE}
...
The resulting plot shows that the 3rd codon position has higher entropy values. This suggests
that if we use this aligment as input for MED it could result in a possible overestimation
of the diversity. 

The next step is about getting to know if the higher entropy values imply a real change in the aminoacid sequence
or are silent substitution. We will use a saturation plot to explore the transitions and transversions ratio. As the NGS datasets that can be used for MED are really large, in this example we will pick 500 random sequences for the analysis (we recommend to use >5000 sequences). For the distance calculation we use the _Kimura's 2-parameters distance_ (K80). We set **all = FALSE** because we only want to focus on the 3rd codon position, we can calculate the ratio for all codon positions with **all = TRUE**.
```{r}
...
For a real world analysis we would perform at least 100 random subsamplings of \>5000 sequences. The parallel version uses [BatchJobs](https://github.com/tudo-r/BatchJobs) to distribute the different random subsamplings over multiple cores or a computer cluster. An example using the different cores of a computer:
```{r}
random_saturation_estimates <- estimate_saturation_n(aln, rep = 10, nseqs = 500, model = "K80", parallel = TRUE, all = FALSE, save_aln = FALSE, conf_file = "~/.BatchJobs.R", reg_id = "rhodo", reg_dir = "~/rhodo_dir")
```
...
After all these steps we can use the exported alignment for the [MED](http://merenlab.org/projects/med/) analysis.

For any questions realted to **oligo4fun** open a new issue [here](https://github.com/genomewalker/oligo4fun/issues)
