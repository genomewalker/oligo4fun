...
plot_saturation_n<-function(aln = aln, rep = 100, nseqs = 1000, model = "K80", parallel = FALSE,
  ...
  if (rep < 2){
    stop("Please use plot_saturation for only one random subsampling", call. = FALSE)
  }
  ...
  if (parallel){
    ...
    function_args <- list(aln = aln, nseqs = nseqs, model = model, all = all, 
                          verbose = FALSE, seed = 0, .rsamp = TRUE,  save_aln = save_aln,
                          dir =  dir)
    ...
    plot_runs <- plyr::llply(1:rep, plot_saturation, ..., aln = aln, nseqs = nseqs, model = model, all = all, verbose = FALSE,
                             seed = 0, .rsamp = TRUE, save_aln = save_aln, dir =  dir, .parallel = F, .progress = plyr::progress_text(width = 80))
  }
  
  results <-list(
  combined_stats = dplyr::rbind_all(lapply(plot_runs, function(x) dplyr::rbind_list(x[["stats"]]))),
  raw = plot_runs,
  plot = lapply(plot_runs, function(x) x[["plot"]]),
  seed = lapply(plot_runs, function(x) x[["seed"]]),
  saturation = lapply(plot_runs, function(x) x[["saturation"]]),
